  ...
  function compile(tElement) {
    ...
    return function postLink(scope) {
      ...
      function getCells(node) {
        // customize by vivek
        var result = [];
        //var result =  Array.prototype.map.call(node.find('th'), function (cell) {
        angular.forEach(node.find('th'),function(cell){
			   result.push(jQLite(cell));
			});
        //~ var result =  Array.from(node.find('th'), function (cell) {
          //~ return jQLite(cell);
        //~ });
